...
from src.shared.cache_redis_client_factory import CacheRedisClientFactory
...
class Initializer:
    ...
    def __init__(self, app: FastAPI, config: Optional[Config] = None) -> None:
        ...
        self._app = app
        self.cache_redis_client_factory = CacheRedisClientFactory(config=self.config)
    ...
    async def __aenter__(self) -> State:
        ...
        # Cache setup
        await self.cache_redis_client_factory.__aenter__()
    ...
    async def __aexit__(
        ...
        await self.cache_redis_client_factory.__aexit__(exc_type, exc_val, exc_tb)
